...
try {
    ...
    $expectedNewLabels=@('Create Desktop Shortcut (New)','Rename Client (New)','Archive Client (New)','Archived Clients (New)','Clean Up Inactive Clients... (New)','Delete Multiple Clients... (New)','Edit Default profile (New)','Client name first in window titles (New)','Mozilla Firefox (New)','Quick tour (New)','Guided walkthrough (New)',"What's new (New)")
    $menuLabels=Get-OptionsMenuLabels $existing;foreach($label in $expectedNewLabels){if(-not$menuLabels.Contains($label)){throw "Options menu omitted '$label': $menuLabels"}}
    ...
    Assert-GuideBodyContains $guide 11 @('Expanded 20-step Quick tour','twelve illustration-only steps','browser slots','Open copied link','Desktop shortcuts','pinned-client and session-tab reordering','Default changes','rename','archive and restore','client-first titles','inactive cleanup','immediate multi-client deletion','without performing their actions') 'Expanded Quick Tour guidance is incomplete.'
    ...
    $announced=@(
        [pscustomobject]@{Id='browsers_restore';Revision=2;Title='Browsers and Restore tabs'},
        [pscustomobject]@{Id='sessions';Revision=3;Title='Open browser windows'},
        [pscustomobject]@{Id='pins_shortcuts';Revision=3;Title='Favorites, links, and shortcuts'},
        [pscustomobject]@{Id='temporary_default';Revision=2;Title='Temporary and starter profiles'},
        [pscustomobject]@{Id='identity';Revision=3;Title='Icons and window titles'},
        [pscustomobject]@{Id='organize';Revision=2;Title='Rename and archive clients'},
        [pscustomobject]@{Id='cleanup';Revision=2;Title='Cache and permanent deletion'},
        [pscustomobject]@{Id='updates';Revision=3;Title='New since 5.2.0.14'}
    )
    $guide=Open-Guide $existing 41018;Assert-Guide $guide 8 'Next' 'ctSpaces Guided Walkthrough';Close-Guide $guide
    foreach($topic in $announced){$expected=if($topic.Id-eq'updates'){'0'}else{''};Assert-Eq (Ini $existing ('read_'+$topic.Id)) $expected "What's New Skip acknowledged $($topic.Id)."}

    $guide=Open-Guide $existing 41018;Assert-Guide $guide 8 'Next' 'ctSpaces Guided Walkthrough';Assert-Eq ([GuideUiQa]::ControlText([GuideUiQa]::GetDlgItem($guide,1302))) $announced[0].Title "What's New first topic mismatch."
    [void][GuideUiQa]::SendMessageW($guide,0x111,[IntPtr]1,[IntPtr]::Zero);Wait-Until{(Ini $existing 'read_browsers_restore')-eq'2'} "What's New Next did not mark only Browsers."|Out-Null;Assert-Eq (Ini $existing 'read_sessions') '' "What's New Next marked Sessions early.";Close-Guide $guide
    $afterFirstLabels=Get-OptionsMenuLabels $existing;if($afterFirstLabels.Contains('Mozilla Firefox (New)')){throw 'Reading Browsers did not clear its Firefox indicator.'};foreach($label in @('Create Desktop Shortcut (New)','Edit Default profile (New)','Rename Client (New)','Clean Up Inactive Clients... (New)',"What's new (New)")){if(-not$afterFirstLabels.Contains($label)){throw "Reading Browsers cleared unrelated '$label'."}}

    $remaining=@($announced|Select-Object -Skip 1);$guide=Open-Guide $existing 41018;Assert-Guide $guide 7 'Next' 'ctSpaces Guided Walkthrough'
    for($index=0;$index-lt$remaining.Count;$index++){
        ...
        if($index+1-lt$remaining.Count){$nextUnreadExpected=if($remaining[$index+1].Id-eq'updates'){'0'}else{''};Assert-Eq (Ini $existing ('read_'+$remaining[$index+1].Id)) $nextUnreadExpected "What's New marked the next topic early.";Wait-Until{[GuideUiQa]::ControlText([GuideUiQa]::GetDlgItem($guide,1302))-eq$remaining[$index+1].Title} "What's New did not advance to the next topic."|Out-Null}
    }
