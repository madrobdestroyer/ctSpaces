...
function Assert-Guide([IntPtr]$Guide,[int]$Topics,[string]$NextLabel,[string]$Caption){
    ...
    foreach($id in @(1301,1302,1303,1304,1305,1306,1,2)){if([GuideUiQa]::GetDlgItem($Guide,$id)-eq [IntPtr]::Zero){throw "Guide control $id missing."}}
    Assert-Eq ([GuideUiQa]::ControlText([GuideUiQa]::GetDlgItem($Guide,1))) $NextLabel 'Next button label mismatch.'
    $client=[GuideUiQa+Rect]::new();[void][GuideUiQa]::GetClientRect($Guide,[ref]$client); foreach($id in @(1301,1302,1303,1304,1305,1306,1,2)){$r=[GuideUiQa]::ChildRect($Guide,$id);if($r.Left-lt 0-or$r.Top-lt 0-or$r.Right-gt$client.Right-or$r.Bottom-gt$client.Bottom){throw "Guide control $id clips outside the client area."}}
...
try {
    ...
    $guide=Open-Guide $existing -F1; $marineShot=Save-Capture $guide 'marine-manual'; Assert-Guide $guide 12 'Next' 'ctSpaces Guided Walkthrough'
